...
void ImageToWorld(cv::Point2d LeftFeatureCoordinates, cv::Point2d RightFeatureCoordinates, cv::Mat Encoders, cv::Mat LeftCamCalibMat, cv::Mat RightCamCalibMat,
				  cv::Point3d &WorldPoint){

	KinTransforms KinTr;

	//Kinematics
	Kinematics_icubEyes Kin;
	Kin.Apply(Encoders,KinTr);

	Mat Pleft = ProjectionMat(LeftCamCalibMat, KinTr.UnifiedRefFrame_To_LeftCamRefFrame);
	Mat Pright = ProjectionMat(RightCamCalibMat, KinTr.UnifiedRefFrame_To_RightCamRefFrame);
...
///Image Features to World Feature
void ImageToWorld(cv::Point2d LeftFeatureCoordinates, cv::Point2d RightFeatureCoordinates, cv::Mat UnifiedToLeft, cv::Mat UnifiedToRight, cv::Mat LeftCamCalibMat, cv::Mat RightCamCalibMat,
				  cv::Point3d &WorldPoint){
